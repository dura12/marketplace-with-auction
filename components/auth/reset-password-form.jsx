...
export function ResetPasswordForm({ email, onSuccess, onCancel }) {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      if (!password || !confirmPassword) {
        toast({ variant: "destructive", title: "Error", description: "Please enter both password fields" });
        setIsLoading(false);
        return;
      }
      ...
      if (passwordErrors.length > 0) {
        toast({ variant: "destructive", title: "Password Requirements", description: passwordErrors[0] });
        setIsLoading(false);
        return;
      }
      ...
      toast({ variant: "destructive", title: "Reset Failed", description: "An error occurred. Please try again." });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleResetPassword} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="password" type={showPassword ? "text" : "password"} className="pl-10 pr-10" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} />
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Lock className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
              <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} className="pl-10 pr-10" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required minLength={8} />
